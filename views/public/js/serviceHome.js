...
window.addEventListener("load", ()=>{
    ...
    const showCustomersResult = () => {
        ...
        if(state.customers.length === 0){//IF IT HAS NO RESULTS TO DISPLAY
            ...
            str += state.customers.map((customer, i)=>`
    ...
    const setCollapsedCustomersListToFalse = () => {
        ...
        state.collapseCustomers = collapsedList.map(_=>false);
    };

    window.addListenerToListItem = (index) => {
    ...
    window.updateBookingStatus = (customerIndex, bookingIndex) => {
    ...
    window.cancelBooking = (customerIndex, booking_index) => {
